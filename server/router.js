...
router.post('/batchUpload', upload.single('file'), function (req, res, next) {
  ...
  res.json({
    ...
    name: file.originalname,
    href: file.path,
    url: 'http://localhost:8089/' + file.path.slice(7),
    alt: file.name
  });
});
...
router.get("/goods/item/insertTbItem", (req, res) => {
  ...
  const sql = "insert into project(title, image, sellPoint, price, cid, category, num, descs, paramsInfo,barcode,status,updated) values (?,?,?,?,?,?,?,?,?,?,?,?)"
  var arr = [title, image, sellPoint, price, cid, category, num, descs, paramsInfo, barcode, status, updated];
  sqlFn(sql, arr, result => {
    if (result.affectedRows > 0) {
      res.send({
        status: 200,
...
router.get("/goods/item/updateTbItem", (req, res) => {
  ...
  var arr = [title, sellPoint, price, cid, category, num, desc, paramsInfo, image, id, barcode, status];
  sqlFn(sql, arr, result => {
    if (result.affectedRows > 0) {
      res.send({
        status: 200,
